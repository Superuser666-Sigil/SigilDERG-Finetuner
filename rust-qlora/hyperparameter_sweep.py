...
def main():
    ...
    ap = argparse.ArgumentParser(description="Run hyperparameter sweep for QLoRA fine-tuning")
    ap.add_argument(
        "--base-cfg", default="configs/llama8b-phase1.yml", help="Base configuration file"
    )
    ap.add_argument(
        "--sweep-dir", default="sweeps", help="Directory to save sweep configurations and results"
    )
    ap.add_argument(
        "--dry-run", action="store_true", help="Print sweep configurations without running training"
    )
    ap.add_argument("--seed", type=int, default=42, help="Random seed for sweep reproducibility")
    ap.add_argument(
        "--timeout",
        type=int,
        default=None,
