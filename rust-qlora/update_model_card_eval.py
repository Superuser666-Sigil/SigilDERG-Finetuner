...
def update_model_card_readme(readme_path, metrics, repo_id=None, checkpoint_name=None):
    ...
    if not metadata["model-index"]:
        ...
        metadata["model-index"] = [{"name": "rust-code-model", "results": []}]
        ...
        if key in metrics and metrics[key] is not None:
            ...
            if metric_type in existing_metrics:
                ...
                rust_eval_result["metrics"].append(
                    {"name": name, "type": metric_type, "value": value}
                )

    # Update the metrics list
    rust_eval_result["metrics"] = list(existing_metrics.values()) + [
        m for m in rust_eval_result["metrics"] if m["type"] not in existing_metrics
    ]
    ...
    updated_markdown = update_evaluation_section(
        markdown_content, metrics, repo_id, checkpoint_name
    )
...
def update_evaluation_section(markdown_content, metrics, repo_id=None, checkpoint_name=None):
    ...
    eval_summary.append(
        f"### Aggregate Metrics (checkpoint {checkpoint_step}, {metrics.get('evaluated_samples', metrics.get('total_samples', 'N/A'))} samples)\n"
    )
    ...
    if repo_id and checkpoint_name:
        ...
        eval_summary.append("")
        eval_summary.append(
            f"- [Metrics (JSONL)]({repo_url}/blob/main/{checkpoint_name}/metrics.jsonl)"
        )
        eval_summary.append(
            f"- [Error Logs (JSONL)]({repo_url}/blob/main/{checkpoint_name}/errors.jsonl)"
        )
        ...
        eval_summary.append("")
        eval_summary.append(
            "- [Metrics (JSONL)](https://huggingface.co/Superuser666-Sigil/Llama-3.1-8B-Instruct-Rust-QLora/blob/main/checkpoint-{checkpoint_step}/metrics.jsonl)"
        )
        eval_summary.append(
            "- [Error Logs (JSONL)](https://huggingface.co/Superuser666-Sigil/Llama-3.1-8B-Instruct-Rust-QLora/blob/main/checkpoint-{checkpoint_step}/errors.jsonl)"
        )
