...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Number of samples to generate per prompt (default: auto-calculated to ensure enough samples)",
    )
    ap.add_argument(
        "--min-total-samples",
        type=int,
        default=64,
        help="Minimum total samples to generate (default: 64)",
    )
    ap.add_argument(
        "--prompts-file",
        type=str,
        default=None,
        help="Path to YAML or JSON file containing prompts (default: use built-in prompts)",
    )
    ap.add_argument(
        "--max-new-tokens",
        type=int,
        default=4096,
        help="Maximum number of new tokens to generate (default: 4096, increased for longer Rust programs)",
    )
    args = ap.parse_args()
    ...
    print(
        f"Generating {samples_per_prompt} samples per prompt ({num_prompts} prompts = {samples_per_prompt * num_prompts} total)"
    )
    ...
    for p in prompts:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a Rust code generator. Output only valid Rust code, wrapped in ```rust code blocks. No explanations or comments outside code blocks.",
            },
            {"role": "user", "content": p},
        ]
        ...
        for sample_idx in range(samples_per_prompt):
            ...
            if snip_clean:
                ...
                if (
                    incomplete_macro
                    or incomplete_string
                    or incomplete_char
                    or incomplete_endings
                    or open_braces > 0
                    or open_parens > 0
                    or open_brackets > 0
                ):
                    ...
                    for i in range(len(lines) - 1, -1, -1):
                        ...
                        is_balanced = (
                            prefix_braces == 0
                            and prefix_parens == 0
                            and prefix_brackets == 0
                            and prefix_macros_unmatched == 0
                        )
                        ends_properly = (
                            line.endswith("}")
                            or line.endswith(";")
