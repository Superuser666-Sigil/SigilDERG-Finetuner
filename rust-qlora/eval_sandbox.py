...
def run_cargo_in_docker(
    ...
    check_result = subprocess.run(
        ["docker", "images", "-q", "rust-eval-sandbox"], capture_output=True, text=True
    )
    ...
    base_docker_opts = [
        ...
        "--tmpfs",
        "/tmp:rw,exec,nosuid,size=300m,mode=1777",  # Temporary writable space with exec permission and proper mode
        "-v",
        f"{project_path}:/eval/{project_name}:ro",  # Mount project as read-only (SECURE - no host filesystem writes)
        "-w",
        f"/eval/{project_name}",  # Working directory
        "-e",
        "CARGO_TARGET_DIR=/tmp/cargo-target",  # Set cargo to use tmpfs for build artifacts
    ]
    ...
    try:
        ...
        if result.returncode != 0 and capture_output and allow_network_fallback:
            ...
            if is_network_error:
                ...
                print(
                    "Network error detected, retrying with network access enabled...",
...
def run_cargo_with_firejail(
    project_path: str, command: list[str], timeout: int = 30, capture_output: bool = True
) -> subprocess.CompletedProcess:
    """
...
def run_cargo_sandboxed(
    ...
    if sandbox_mode == "docker":
        return run_cargo_in_docker(
            project_path, command, timeout, capture_output, allow_network_fallback
        )
        ...
        return subprocess.run(
            command, cwd=project_path, capture_output=capture_output, text=True, timeout=timeout
        )
    else:
