...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="out/merged", help="Output directory for merged model"
    )
    parser.add_argument(
        "--base-model",
        type=str,
    ...
    try:
        ...
        stat = shutil.disk_usage(
            args.output if os.path.isabs(args.output) else os.path.dirname(args.output) or "."
        )
        free_gb = stat.free / (1024**3)
        if free_gb < 5:
            print(
                f"Warning: Low disk space ({free_gb:.1f} GB free). Model export may require ~5-10 GB."
            )
    except Exception:
    ...
    try:
        ...
        if os.path.exists(adapter_config_path):
            ...
            if checkpoint_base_model and checkpoint_base_model != args.base_model:
                print(
                    f"Warning: Checkpoint was trained with base model '{checkpoint_base_model}', "
    ...
    config = m.config
    print(
        f"Model config: {config.model_type}, vocab_size={config.vocab_size}, "
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to create output directory '{args.output}': {e}. Check permissions."
        )
